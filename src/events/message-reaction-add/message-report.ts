...
const event: MessageReactionAddEvent = {
    execute: async (client, reaction) => {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                {name: 'Message', value: reaction.message.content ?? 'No content', inline: true},
                {name: 'Author', value: reaction.message.author?.toString() ?? 'No author', inline: true},
                {name: 'Link', value: `[Jump to message](${reaction.message.url})`, inline: true },
            )
